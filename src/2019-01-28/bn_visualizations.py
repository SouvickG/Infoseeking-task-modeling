...
exp1_combined_general = pd.read_csv('/Users/Matt/Desktop/Experiment/data/interim/structure_graph_metrics/exp1_combineddataset_metrics.csv')
exp1_combined_general = exp1_combined_general[exp1_combined_general['feature']=='all']

exp1_combined_general = exp1_combined_general[exp1_combined_general['percent_training']==100]
print(exp1_combined_general['percent_training'])

exp1_exclusive_general = pd.read_csv('/Users/Matt/Desktop/Experiment/data/interim/structure_graph_metrics/exp1_onedataset_metrics.csv')
exp1_exclusive_general = exp1_exclusive_general[exp1_exclusive_general['feature']=='all']
exp1_exclusive_general = exp1_exclusive_general[exp1_exclusive_general['percent_training']==80]

print(len(exp1_combined_general.index))
print(exp1_exclusive_general)
df = []
FEATURES = [
...
for (t1,t2) in zip(train_datasets_allcolumns,test_datasets_allcolumns):
    scores = exp1_combined_general[(exp1_combined_general['training_set']==t1) & (exp1_combined_general['test_set']==t2)]
    sc = scores['logLik'].mean()
    print(t1,t2,sc)
    df+= [{'train':t1,'test':t2,'score':sc}]

for t in train_datasets_exclusive:
    scores = exp1_exclusive_general[(exp1_exclusive_general['training_set']==t) & (exp1_exclusive_general['test_set']==t)]
    sc = scores['logLik'].mean()
    print(t1,t2,sc)
    df+= [{'train':t,'test':t,'score':sc}]


d1 = exp1_exclusive_general[(exp1_exclusive_general['training_set']=='eopn') & (exp1_exclusive_general['test_set']=='eopn')]
d2 = exp1_combined_general[(exp1_combined_general['training_set']=='sal') & (exp1_combined_general['test_set']=='eopn')]
print(d1['logLik'].mean(),d2['logLik'].mean(),stats.ttest_ind(d1['logLik'],d2['logLik']))

# d1 = exp1_exclusive_general[(exp1_exclusive_general['training_set']=='nsf') & (exp1_exclusive_general['test_set']=='nsf')]
d1 = exp1_combined_general[(exp1_combined_general['training_set']=='nsf') & (exp1_combined_general['test_set']=='eopn')]
d2 = exp1_combined_general[(exp1_combined_general['training_set']=='nsf_sal') & (exp1_combined_general['test_set']=='eopn')]
print(d1['logLik'].mean(),d2['logLik'].mean(),stats.ttest_ind(d1['logLik'],d2['logLik']))

d1 = exp1_exclusive_general[(exp1_exclusive_general['training_set']=='sal') & (exp1_exclusive_general['test_set']=='sal')]
d2 = exp1_combined_general[(exp1_combined_general['training_set']=='nsf_eopn') & (exp1_combined_general['test_set']=='sal')]
print(d1['logLik'].mean(),d2['logLik'].mean(),stats.ttest_ind(d1['logLik'],d2['logLik']))



df = pd.DataFrame(df)
df.to_csv('/Users/Matt/Desktop/df.csv')
df = df[['test','train','score']]
df_pivot = df.pivot(index='train', columns='test', values='score')
# df_pivot = df_pivot.sort_values(by=['train','test'],ascending=[True,True])
# df_pivot.to_csv('/Users/Matt/Desktop/exp1.1generalgraph.csv')
print df.sort_values(by=['test','train'],ascending=[True,True]).to_latex()
...
#Experiment 3
exp3_df = pd.read_csv('/Users/Matt/Desktop/Experiment/data/interim/structure_graph_metrics/exp3_metrics.csv')
exp3_df = exp3_df[exp3_df['feature']=='all']
ax = sns.pointplot(x='percent_missing', y='logLik', hue='training_set',data=exp3_df)
ax.set_title('Missing Data')
fig = ax.get_figure()
#plt.xticks(rotation=45)
plt.tight_layout()
plt.ylim(-3500,0)
fig.savefig("/Users/Matt/Desktop/exp3_all.png")
plt.clf()
...
for f in FEATURES:
    for (t1,t2) in zip(train_datasets_allcolumns,test_datasets_allcolumns):
        print(t1,t2)
        scores = exp1_combined_general[(exp1_combined_general['training_set']==t1) & (exp1_combined_general['test_set']==t2)]
        scores = scores[scores['feature']==f]
        if len(scores.index)>0:
            sc = scores['logLik'].mean()
            print(sc)
            df+= [{'train':t1,'test':t2,'feature':f,'score':sc}]

    for t in train_datasets_exclusive:
        scores = exp1_exclusive_general[(exp1_exclusive_general['training_set']==t) & (exp1_exclusive_general['test_set']==t)]
        scores = scores[scores['feature']==f]
        print("LEN",len(scores.index))
        if len(scores.index)>0:
            sc = scores['logLik'].mean()
            df+= [{'train':t,'test':t,'feature':f,'score':sc}]

df = pd.DataFrame(df)
df.to_csv('/Users/Matt/Desktop/df.csv')
df = df[['test','feature','score','train']]
# df_pivot = df.pivot(index='train', columns='test', values='score')
# df_pivot = df_pivot.sort_values(by=['train','test'],ascending=[True,True])
# df_pivot.to_csv('/Users/Matt/Desktop/exp1.1generalgraph.csv')
print df.sort_values(by=['test','feature','score'],ascending=[True,True,False]).to_latex()
# print df_pivot.to_latex()
...
#2.1 - general graph with constant test size, variable training size
#
##2.1 - general graph with variable test size, constant training size

exp2_combined_general = pd.read_csv('/Users/Matt/Desktop/Experiment/data/interim/structure_graph_metrics/exp2_combineddataset_metrics.csv')
exp2_combined_general = exp2_combined_general[exp2_combined_general['feature']=='all']
exp2_combined_general = exp2_combined_general[exp2_combined_general['percent_test']==80]

exp2_combined_general_2 = pd.read_csv('/Users/Matt/Desktop/Experiment/data/interim/structure_graph_metrics/exp2_combineddataset_metrics_2.csv')
exp2_combined_general_2 = exp2_combined_general_2[exp2_combined_general_2['feature']=='all']
exp2_combined_general_2 = exp2_combined_general_2[exp2_combined_general_2['percent_training']==100]

exp2_exclusive_general = pd.read_csv('/Users/Matt/Desktop/Experiment/data/interim/structure_graph_metrics/exp2_onedataset_metrics.csv')
exp2_exclusive_general = exp2_exclusive_general[exp2_exclusive_general['feature']=='all']

d = exp2_combined_general.copy(deep=True)
d['test_set'] = d['test_set'].map({'nsf':'INT','sal':"SAL",'eopn':'EOPN'})
d['training_set'] = d['training_set'].map({'nsf':'INT','sal':"SAL",'eopn':'EOPN','nsf_sal':'INT,SAL','nsf_eopn':'INT,EOPN','sal_eopn':'SAL,EOPN'})
for (n,group) in d.groupby('test_set'):
    ax = sns.pointplot(x='percent_training', y='logLik', hue='training_set',data=group)
    ax.set_title('Exp 2: Vary Percent External Test Data For %s'%n)
    fig = ax.get_figure()
    plt.tight_layout()
    plt.ylim(-3000,0)
    fig.savefig("/Users/Matt/Desktop/exp2_21_%s.png"%n)
    plt.clf()

# for (n,group) in exp2_combined_general_2.groupby('test_set'):
#     print(group[['training_set','percent_test','logLik']])
#     ax = sns.pointplot(x='percent_test', y='logLik', hue='training_set',data=group)
#     ax.set_title('Exp2.2: Vary Test size')
#     fig = ax.get_figure()
#     plt.tight_layout()
#     fig.savefig("/Users/Matt/Desktop/exp2_2.2_%s.png"%n)
#     plt.clf()

for (n,group) in exp2_exclusive_general.groupby('test_set'):
    ax = sns.pointplot(x='percent_training', y='logLik', hue='training_set',data=group)
    ax.set_title('Exp2.3: Vary Training/Test Split')
    fig = ax.get_figure()
    plt.tight_layout()
    plt.ylim(-3000,0)
    fig.savefig("/Users/Matt/Desktop/exp2_2.3_%s.png"%n)
    plt.clf()


# for (n,group) in exp2_exclusive_general.groupby('training_set'):
#     ax = sns.pointplot(x='percent_training', y='logLik', hue='test_set',data=exp2_exclusive_general)
#     ax.set_title('Exp1: Vary Training/Test Split')
#     fig = ax.get_figure()
#     plt.tight_layout()
#     plt.ylim(-3000,0)
#     fig.savefig("/Users/Matt/Desktop/exp1_control_all.png")
#     plt.clf()
d = exp2_exclusive_general.copy(deep=True)
d = d.rename(columns={"percent_training": "Percent Training", "logLik": "Log Likelihood"})
d['test_set'] = d['test_set'].map({'eopn':'EOPN','sal':'SAL','nsf':'INT'})
print(d['test_set'])
ax = sns.pointplot(x='Percent Training', y='Log Likelihood', hue='test_set',data=d)
ax.set_title('Experiment 1: Training/Test Ratio for one Data Set')
fig = ax.get_figure()
plt.tight_layout()
...
shd_hamming = pd.read_csv('/Users/Matt/Desktop/Experiment/data/interim/structure_graph_metrics/shd_hamming.csv')
i = 0
d_shd = []
d_hamming = []
for (n,group) in shd_hamming.groupby(['train_dataset_name_1','test_dataset_name_1','train_dataset_name_2','test_dataset_name_2']):
    print("GROUP",n)
    train_test_1_string = "%s/%s"%(n[0],n[1])
    train_test_2_string = "%s/%s"%(n[2],n[3])
    print("\t",len(group.index))

    d_shd += [{'ds1':train_test_1_string,'ds2':train_test_2_string,'shd':round(group['shd'].mean(),2)}]
    d_hamming += [{'ds1':train_test_1_string,'ds2':train_test_2_string,'hamming':round(group['hamming'].mean(),2)}]
    i+=1

print(pd.DataFrame(d_shd).pivot(index='ds1', columns='ds2', values='shd').to_latex())
print(pd.DataFrame(d_hamming).pivot(index='ds1', columns='ds2', values='hamming').to_latex())
print(i)
exit()
