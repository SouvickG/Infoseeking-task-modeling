...
base_folder = '../../reports/accuracy_slices/'



accuracy_dataframes = dict()

for feature in ['lit','full']:
    for dataset in ['active','full']:
        for personalized in ['true','false']:
            dataframe_filename = base_folder+'results_features_%s_dataset_%s_personalized_%s/prediction_lr_scoreresults.csv'%(feature,dataset,personalized)
            if os.path.exists(dataframe_filename):
                accuracy_dataframes[(feature,dataset,personalized)] = pd.DataFrame.from_csv(dataframe_filename)


print(len(accuracy_dataframes.keys()))
print(accuracy_dataframes.keys())


comparisons = [
#     (('lit','full','false','allfeatures_knn','facet_goal'),('full','full','false','stratified','facet_goal')),
# (('full','full','false','allfeatures_ada','facet_goal'),('full','full','false','stratified','facet_goal')),
# (('lit','full','false','allfeatures_knn','facet_product'),('full','full','false','mostfrequent_baseline','facet_product')),
# (('full','full','false','allfeatures_svc','facet_product'),('full','full','false','mostfrequent_baseline','facet_product')),
# (('lit','full','false','allfeatures_knn','facet_trec2014tasktype'),('full','full','false','stratified','facet_trec2014tasktype')),
# (('full','full','false','allfeatures_svc','facet_trec2014tasktype'),('full','full','false','stratified','facet_trec2014tasktype')),
#
# (('lit','active','false','stratified','facet_goal'),('full','active','false','stratified','facet_goal')),
# (('full','active','false','allfeatures_svc','facet_goal'),('full','active','false','stratified','facet_goal')),
# (('lit','active','false','stratified','facet_product'),('lit','active','false','stratified','facet_product')),
# (('full','active','false','allfeatures_knn','facet_product'),('lit','active','false','stratified','facet_product')),
# (('lit','active','false','stratified','facet_trec2014tasktype'),('full','active','false','stratified','facet_trec2014tasktype')),
# (('full','active','false','allfeatures_ada','facet_trec2014tasktype'),('full','active','false','stratified','facet_trec2014tasktype')),
#
# (('lit','active','true','allfeatures_gnb','facet_goal'),('lit','active','true','stratified','facet_goal')),
# (('full','active','true','allfeatures_gnb','facet_goal'),('lit','active','true','stratified','facet_goal')),
# (('lit','active','true','allfeatures_ada','facet_product'),('full','active','true','stratified','facet_product')),
# (('full','active','true','allfeatures_ada','facet_product'),('full','active','true','stratified','facet_product')),
# (('lit','active','true','allfeatures_gnb','facet_trec2014tasktype'),('lit','active','true','stratified','facet_trec2014tasktype')),
# (('full','active','true','allfeatures_gnb','facet_trec2014tasktype'),('lit','active','true','stratified','facet_trec2014tasktype')),
#
# (('lit','active','false','allfeatures_knn','userID'),('full','active','false','mostfrequent_baseline','userID')),
# (('full','active','false','allfeatures_knn','userID'),('full','active','false','mostfrequent_baseline','userID')),


(('full','full','false','allfeatures_svc','facet_trec2014tasktype'),('full','active','true','allfeatures_gnb','facet_trec2014tasktype')),



    (('full','active','false','allfeatures_ada','facet_trec2014tasktype'),('full','active','true','allfeatures_gnb','facet_trec2014tasktype'))

               ]


for (c1,c2) in comparisons:
    (feature1,dataset1,personalized1,algorithm1,target1) = c1
    (feature2,dataset2,personalized2,algorithm2,target2) = c2

    frame1 = accuracy_dataframes[(feature1,dataset1,personalized1)]
    frame2 = accuracy_dataframes[(feature2, dataset2, personalized2)]
    accuracy_scores_1 = frame1[(frame1['featureset']==algorithm1)&(frame1['intention']==target1)]['accuracy']
    accuracy_scores_2 = frame2[(frame2['featureset']==algorithm2)&(frame2['intention']==target2)]['accuracy']

    (stat, p) = (None,None)
    (stat1, pnorm1) = stats.normaltest(accuracy_scores_1)
    (stat2, pnorm2) = stats.normaltest(accuracy_scores_2)
    if (pnorm1 < .05 or pnorm2 < .05):
        (stat, p) = stats.mannwhitneyu(accuracy_scores_1, accuracy_scores_2)
    else:
        print("normal ttest")
        (stat, p) = stats.ttest_ind(accuracy_scores_1, accuracy_scores_2)

    print(c1,c2,np.mean(accuracy_scores_1),np.mean(accuracy_scores_2),p)
    if (p<.05):
        print("SIGNIFICANT!",p,c1,c2,np.mean(accuracy_scores_1),np.mean(accuracy_scores_2))
...
def run_logisticregression(df,input_features=None,clf=None):
    ...
    # weights = dict([(f,[]) for f in FEATURES])
        ...
        for i in range(NUM_ITERS):
            ...
            for u in userIDs:
                ...
                while True:
                    ...
                    try:
                        # print("Fit Prediction Transformer")
                        # print(df)
                ...
                # 2) LogisticRegression
                        if input_features==['dummy1']:
                            ...
                            anovakbest_filter = SelectKBest(f_regression, k=min([10, len(features)]))
                            # lr_model = mlp
                            clf_map={'knn': knn,
                                     'gnb': gnb,
                                     'svm': svm,
                            ...
                            # lr_model = LogisticRegressionCV(cv=3,random_state=random.randint(1,4294967294))
                ...
                # print("percents",[f(label,y_pred) for label in set(list(y_pred))])

                # print(X_test)
                # print(lr_model.predict_proba(X_test)[:,])
                y_score = lr_model.predict_proba(X_test)[:,1]

                # for (ind,fname) in zip(range(len(features)),features):
                #     res["weight_"+fname] = lr_model.coef_[0][ind]
                ...
                # res['y_true'] = le.inverse_transform(y_test)
                # res['y_pred'] = le.inverse_transform(y_pred)
                # # res['y_true'] = y_test
                # # res['y_pred'] = y_pred
                # res['y_score'] = y_score
                # res['run_ct'] = RUN_CT
...
if __name__=='__main__':
    ...
    # active_users = list(set(sessions_per_user_df[sessions_per_user_df['num_sessions']>=20]['user_num'].tolist()))
    # active_users = list(set(sessions_per_user_df[sessions_per_user_df['num_sessions']>=20]['user_num'].tolist()))
    # active_users = list(set(sessions_per_user_df[sessions_per_user_df['user_num'] == 18]['user_num'].tolist()))
    ...
    for (featuregroupname,featuregroup,clf) in featurenames_and_featuresets:
        ...
        for r in lr_res:
            ...
            # y_true,y_pred,y_score,n_queries = r.pop('y_true'),r.pop('y_pred'),r.pop('y_score'),r.pop('n_queries')
            # y_true,y_pred,y_score,n_queries = pd.Series(y_true),pd.Series(y_pred),pd.Series(y_score),pd.Series(n_queries)
            ...
            # 'y_true':y_true,'y_pred':y_pred,'y_score':y_score,'n_queries':n_queries})
