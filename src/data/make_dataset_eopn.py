...
class EOPNLogParser:
    ...
    valid = False
    connection = None
    dbname = 'spring2017_expert'
    dbhost = 'localhost'
    dbuser = 'root'
    dbpwd = 'root'
    substring_coagmento = 'coagmento.org'
    substring_etherpad = 'coagmentopad.rutgers.edu'
    substrings_badurls = ['facebook.com', 'buzzfeed.com', 'rutgers.edu','mail.google.com','tumblr.com',
                          'youtube.com','acnestudios','pof.com','twitch.tv','biology','engineering',
                          'calendar.google.com','glassdoor.com','humanbenchmark.com',
                          'imgur.com','reddit.com/r/nosleep','reddit.com/r/photoshopbattles','iflmylife.com','twitter.com',
                          'thesaurus.com','livepolish.com','instagram.com','mensa.org','cambridgebrainsciences.com','collegeboard.org',
                          'iqcomparisonite.com','putlockeris.org','radaris.com','rutgersonline.net','rutgers.edu','ecollege.com',
                          'watchcommunity.tv','watchseries.cx','weibo.com','wowmovie.me','acnestudioes.com','chanel.com',
                          'cookinglight.com','coastalliving.com','deviantart.com','foodandwine.com',
                          'geekrank.com','iflmylife.com','iqcomparisonite.com','musiciansfriend.com',
                          'preziouz.com','accounts.google.com','calendar.google.com','craigslist.org','docs.google.com',
                          'datemyschool.com','drive.google.com','hackensackuhn','minecraft.net','myaccount.google.com',
                          'myaccount.allstate.com','spotify.com','putlockeris','us.battle.net','dominos.com','epicgames.com',
                          'ezpassnj.com','linkedin.com','netflix.com',
    ...
    skip_sessions = [(39, 1), (39, 2), (56, 1), (56, 2), (80, 1)]  # (userID,taskNum of tasks to skip)
    ...
    invalid_queryurls = None #TODO
    invalid_pageIDs = None # TODO: Don't worry about this for now. Next time
    invalid_queryIDs_fileloc = '/Users/Matt/Documents/Programming/GitHub/Repositories/task-modeling/data/interim/eopn_bad_queries.csv'
    ...
    # questionnaire_columns_demographic = ['perceived_difficulty', 'perceived_complexity', 'task_understanding',
    #                                    'topic_knowledge', 'sufficient_time', 'anticipated_success']

    session_to_presearch = None
    questionnaire_columns_presearch = ['perceived_difficulty','perceived_complexity','task_understanding','topic_knowledge','sufficient_time','anticipated_success']
    session_to_postsearch = None
    questionnaire_columns_postsearch = ['task_understanding','actual_difficulty','sufficient_time','perceived_success_gathering','topic_knowledge']
    session_to_pretasklab = None
    questionnaire_columns_pretasklab = ['always_find','always_findmeans','persist_goals','complex_confident','handle_unforeseen','solve_witheffort','remain_calm','moreinfo_thanothers','lesstime_thanothers','search_likeexpert','morepatience_thanothers','athome_knowledge','atwork_knowledge','otherproblems_knowledge']
    ...
    def __init__(self, output_csv='./output.csv'):
        ...
        self.session_to_pretasklab = self._extract_questionnaire_data('questionnaire_pretasklab',
                                                                     self.questionnaire_columns_pretasklab)
    ...
    def _get_session_metadata(self, userID, taskNum):
        ...
        if(taskNum in [3]):
            to_return.update(self.session_to_pretasklab[(userID, taskNum)])
        else:
            to_return.update(self.session_to_pretasklab[(userID, 3)])
        return to_return
    ...
    def _create_feature_vector(self, current_state, previous_state, total_state):
        ...
        f_vec['task_page'] = current_state['bad_query_segment']
        ...
        f_vec['task_page'] = bool((self.substring_coagmento in f_vec['current_tab']) or (self.substring_etherpad in f_vec['current_tab']) or (
                        sum([sub in f_vec['current_tab'] for sub in self.substrings_badurls]) > 0))
        if (not f_vec['task_page']):
            if 'coagmento' in f_vec['current_tab']:
                print("NOT TASK PAGE")
                pprint.pprint(current_state['tabs'])
                pprint.pprint(previous_state['tabs'])
                print(self.substring_coagmento in current_state['tabs'][0])
                print(self.substring_etherpad in current_state['tabs'][0])
                print(self.substring_coagmento in previous_state['tabs'][0])
                print(self.substring_etherpad in previous_state['tabs'][0])
                exit()
        ...
        f_vec['search_taskdomain_1'] = total_state['moreinfo_thanothers']
        f_vec['search_taskdomain_2'] = total_state['lesstime_thanothers']
        f_vec['search_taskdomain_3'] = total_state['search_likeexpert']
        f_vec['search_taskdomain_4'] = total_state['morepatience_thanothers']
        print([f_vec['search_taskdomain_%d'%i] for i in range(1,5)])

        f_vec['search_taskdomain'] = np.mean([int(f_vec['search_taskdomain_%d'%i]) for i in range(1,5)])

        f_vec['search_expertise_1'] = total_state['solve_witheffort']
        f_vec['search_expertise_2'] = total_state['athome_knowledge']
        f_vec['search_expertise_3'] = total_state['atwork_knowledge']
        f_vec['search_expertise_4'] = total_state['otherproblems_knowledge']
        print([f_vec['search_expertise_%d' % i] for i in range(1, 5)])
        f_vec['search_expertise'] = np.mean([int(f_vec['search_expertise_%d' % i]) for i in range(1, 5)])

        f_vec['topic_familiarity'] = total_state['topic_knowledge']
        f_vec['post_difficult'] = total_state['actual_difficulty']
        f_vec['post_rushed'] = total_state['sufficient_time']
        ...
        for seg_session in ['segment']:
            for pagetype in ['serp', 'content']:
                ...
                # TODO: ,'views_total', 'time_dwellfirst'
        ...
        f_vec['pages_num_segment'] = sum(
            [((current_state['pagedata_segment'][url]['views_total'] > 0)and('google.com/webhp' not in url)and('google.com/search' not in url)) for url in segment_urls])

        session_urls = set(
            [url for url in total_state['pagedata_total'].keys() if url not in ['about:blank', 'about:newtab', '']])
        f_vec['pages_num_session'] = sum(
            [((total_state['pagedata_total'][url]['views_total'] > 0)and('google.com/webhp' not in url)and('google.com/search' not in url)) for url in session_urls])
    ...
    def slice_frame(self, override=False):
        ...
        self.auxiliary_datasets['byquery_cleaned'] = self.auxiliary_datasets['byquery_cleaned'][self.auxiliary_datasets['byquery_cleaned']['bad_query_segment']==False]
        return self.auxiliary_datasets


# Done
if __name__ == '__main__':
    logger = EOPNLogParser(output_csv='/Users/Matt/Desktop/features_eopn_alldata.csv')
    logger.process_log()
    subframes = logger.slice_frame()
    subframes['byquery'].to_csv('/Users/Matt/Desktop/features_eopn_byquery.csv')
    subframes['byquery_cleaned'].to_csv('/Users/Matt/Desktop/features_eopn_byquery_cleaned.csv')
    exit()
#
