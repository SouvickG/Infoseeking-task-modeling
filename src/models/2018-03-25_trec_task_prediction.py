...
def run_logisticregression(df,input_features=None,clf='str',num_iters=60):
    global FEATURES
    ...
    for facet in PREDICTIONS:
        print(facet)
        label_encoder.fit(df[facet].as_matrix())
        for i in range(num_iters):
            print(i)
            data_okay = True
            while True:
                try:
                    ...
                    clf_map={
                        'str':DummyClassifier(strategy='stratified',random_state=random.randint(1,4294967294)),
                        'mfq':DummyClassifier(strategy='most_frequent', random_state=random.randint(1, 4294967294)),
                        'knn': KNeighborsClassifier(n_neighbors=10),
                        'gnb': GaussianNB(),
                        'rfc': RandomForestClassifier(random_state=random.randint(1, 4294967294)),
                        'ada': AdaBoostClassifier(random_state=random.randint(1, 4294967294)),
                        'qda': QuadraticDiscriminantAnalysis(),
                        'svc': SVC(probability=True, gamma=0.7, C=1, random_state=random.randint(1, 4294967294)),
                        'mlp': MLPClassifier(alpha=1),
                        'ovr':OneVsRestClassifier(LinearSVC(random_state=random.randint(1, 4294967294))),
                        'ovo':OneVsOneClassifier(LinearSVC(random_state=random.randint(1, 4294967294))),
                        'gsc':GridSearchCV(LinearSVC(),{'dual':[True,False],'C':[0.1,1, 10]}),
            ...
            res['y_true'] = label_encoder.inverse_transform(y_test)
            res['y_pred'] = label_encoder.inverse_transform(y_pred)
            results += [res]
    return results
...
if __name__=='__main__':
    ...
    prediction_dataset = dataset_lastqueryonly
    ...
    for (featuregroupname,featuregroup,clf) in featurenames_and_featuresets:
        ...
        lr_res = run_logisticregression(prediction_dataset,input_features=featuregroup,clf=clf,num_iters=60)
        RUN_CT += 1
    ...
    result_dump_dataframe.to_csv('../../reports/prediction_lr_fulldump.csv')
